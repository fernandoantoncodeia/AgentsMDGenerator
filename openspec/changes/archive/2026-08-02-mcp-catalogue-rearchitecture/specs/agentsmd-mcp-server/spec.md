...
## Purpose
The MCP server is the catalogue service for the AgentsMDGenerator ecosystem. It exposes the curated and proposed catalogue entries as MCP resources and tools, runs no LLM logic, and does not scan consumer project files. The server lives in the AgentsMDGenerator master repo and serves connected projects via the Model Context Protocol.
