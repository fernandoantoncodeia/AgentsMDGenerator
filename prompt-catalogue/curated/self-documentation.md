...
- Keep ADRs short and dated; one decision per file. Status: proposed, accepted, superseded, deprecated. For Openspec-drive repos, ADRs MUST be linked to the matching specs.
- For OpenSpec-driven repos, link ADRs to the matching `openspec/changes/` slug so reviewers can find both. 
- When applicable, document functional flows using mermaid sequence diagrams. For Openspec-drive repos, diagrams MUST be linked to the matching specs. 
